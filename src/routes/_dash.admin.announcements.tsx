  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Announcements — Admin" },
      {
        ...
        content: "Create, pin and remove announcements and notifications.",
      },
    ],
  }),
...
function AnnouncementsPage() {
  ...
  const cta = useMemo(
    () =>
      ctaLabel.trim() && ctaTo.trim() ? { label: ctaLabel.trim(), to: ctaTo.trim() } : undefined,
    [ctaLabel, ctaTo],
  );
  ...
  const submit = () => {
    ...
    if (editingId) {
      ...
      track("announcements", `updated the announcement "${title.trim()}"`, null, {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Announcements"
        description="Compose announcements and notifications, and manage what's already been sent."
      />
      ...
      <section className="surface-card rise-in space-y-4 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Bake sale this Friday"
            />
          </div>
          <div className="space-y-1.5">
            <Label>Body</Label>
            ...
            >
              {(["announcement", "notification", "both"] as const).map((v) => (
                <div key={v} className="flex items-center gap-2">
                  ...
                  <Label htmlFor={`d-${v}`} className="capitalize font-normal">
                    {v}
                  </Label>
                </div>
              ))}
        ...
        {!editingId && delivery !== "announcement" && (
          <div className="space-y-2">
            ...
            <RadioGroup
              value={targetMode}
              onValueChange={(v) => setTargetMode(v as "all" | "some")}
              className="flex gap-4"
            >
              <div className="flex items-center gap-2">
                <RadioGroupItem value="all" id="t-all" />
                <Label htmlFor="t-all" className="font-normal">
                  Everyone
                </Label>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="some" id="t-some" />
                <Label htmlFor="t-some" className="font-normal">
                  Specific members
                </Label>
              </div>
            </RadioGroup>
      ...
      {([false, true] as const).map((archivedView) => {
        const list = [...db.announcements]
          .filter((a) => !!a.archived === archivedView)
          .sort((a, b) => b.ts - a.ts);
          ...
          <section key={String(archivedView)} className="surface-card rise-in space-y-3 p-5">
            ...
            <ul className="space-y-2">
              {list.map((a) => (
                <li
                  key={a.id}
                  className="flex items-start gap-3 rounded-xl bg-secondary/50 p-3 transition-colors hover:bg-secondary/70"
                >
                  ...
                  {!archivedView && (
                    <button
                      ...
                      onClick={() =>
                        setDB((d) => {
                          const t = d.announcements.find((x) => x.id === a.id);
                  ...
                  <button
                    ...
                    onClick={() => {
                      setDB((d) => {
                        const t = d.announcements.find((x) => x.id === a.id);
                        if (t) t.archived = !archivedView;
                      });
                      toast.success(
                        archivedView ? "Announcement restored." : "Announcement archived.",
                      );
                    }}
                  >
                    {archivedView ? (
              ...
              <li key={n.id} className="flex items-start gap-3 rounded-xl bg-secondary/50 p-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    {n.targets === "all"
                      ? "Everyone"
                      : n.targets.map((id) => userLabel(db, id)).join(", ") || "No one"}
                  </p>
                </div>
                <button
                  className="rounded-full p-1.5 text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive"
                  onClick={() =>
                    setDB((d) => {
                      d.notifications = d.notifications.filter((x) => x.id !== n.id);
                    })
                  }
                >
                  <Trash2 className="size-4" />
