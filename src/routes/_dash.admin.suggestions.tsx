...
export const Route = createFileRoute("/_dash/admin/suggestions")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Create and target suggestion cards shown on member home pages.",
      },
...
function SuggestionsPage() {
  ...
  const submit = () => {
    ...
    if (editingId) {
      ...
      track("suggestions", `updated the suggestion "${title.trim()}"`, null, {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Suggestions"
        description="Create suggestion cards for member home pages, targeted to everyone or specific people."
      />
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Join the design team"
            />
          </div>
          <div className="space-y-1.5">
            <Label>Body</Label>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="s-some" className="font-normal">
                Specific members
              </Label>
            </div>
          </RadioGroup>
          {targetMode === "some" && (
            <div className="max-h-48 space-y-1 overflow-y-auto rounded-xl border border-border p-2">
              {db.users.map((u) => (
                <label
                  key={u.id}
                  className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-secondary/60"
                >
      ...
      <section className="surface-card rise-in space-y-3 p-5">
        ...
        <ul className="space-y-2">
              ...
              <li key={s.id} className="flex items-start gap-3 rounded-xl bg-secondary/50 p-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    ...
                    {s.targets === "all"
                      ? "Everyone"
                      : s.targets.map((id) => userLabel(db, id)).join(", ") || "No one"}
                  </p>
                  {(() => {
                    const st = suggestionStats(db, s);
                    return (
                      <p className="mt-1 text-xs text-muted-foreground">
                        {st.audience.length} received · {st.completed.length} marked done ·{" "}
                        {st.ignored.length} ignored · {st.pending.length} no response yet
                      </p>
                    );
                  })()}
                  <button
                ...
                <button
                  ...
                  onClick={() =>
                    setDB((d) => {
                      d.suggestions = d.suggestions.filter((x) => x.id !== s.id);
                    })
                  }
                >
          ...
          {db.suggestions.length === 0 && (
            <p className="p-4 text-center text-sm text-muted-foreground">No suggestions yet.</p>
          )}
        </ul>
      </section>
    </div>
